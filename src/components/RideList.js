...
function RideList() {
  ...
  const { user, setIsLoading, setCurrentRide, setSelectedFrom, setSelectedTo } = useContext(Context);
   ...
   useEffect(() => {
    ...
    const rideRef =  firebase.database().ref().child('rides').orderByChild('status').equalTo(0);
  ...
  const acceptRide = (request) => {
    ...
    setIsLoading(true);
    firebase.database().ref(`rides/${request.rideUuid}`).set(request).then(() => {
      setIsLoading(false);
      //set created ride.
  ...
  const renderRideList = () => {
    if (rideRequests && rideRequests.length !== 0) {
      ...
      return (<h3 className="empty-message">
  ...
  return (
    <div className="ride-list">
      <div className="ride-list__container">
        <div className="ride-list__title">
          {/* Ride Requests */}
          Ride offers
          </div>
        <div></div>
