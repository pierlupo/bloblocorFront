...
function RideDetail(props) { 
  ...
  const updateRide = (ride) => { 
    ...
    // realTimeDb.ref(`rides/${ride.rideUuid}`).set(ride).then(() => {
      firebase.database().ref(`rides/${ride.rideUuid}`).set(ride).then(() => {
      setIsLoading(false);
  ...
  return (
    <div className="ride-detail">
      ...
      <div className="ride-detail__actions">
        ...
        <button className="ride-detail__btn" onClick={talkToUser}>{isDriver ? 'Talk to Driver' : 'Talk to User'}</button>
